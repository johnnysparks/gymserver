...
var Emailer = Class.extend({
  ...
  init: function(){
    ...
    this.sendgrid   = new SendGrid( CONFIG.sendgrid  );
    this._to        = [];
  ...
  send: function(fn){
    ...
    if(this._files.length){
      for(file in this._files){
        email.addFile(this._files[file]);
      }
    }
    
    this.sendgrid.sendgrid(this.email, function(message){
      if(err) {
  ...
  attach: function(name, file){
    if(name && file){
      this._files.push({filename: name, content: file});
    } else {
